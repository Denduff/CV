...
namespace PFVision.StreamingProviders
{
    public class Napster
    {
        ...
        {       //make it return list of songs
            ...
            var liste = songList.Where(song => song.streamType.Equals(Song.StreamType.Napster));
            var linqlist = from song in songList
                           where song.streamType.Equals(Song.StreamType.Napster)
                           select song;
        ...
        public IList<Account> GetAccounts()
        {
            ...
            var retlist = accountList.Where<Account>(x => x.streamType == Account.StreamType.Napster);
        ...
        public void ConnectAccounts()
        {
            foreach (var acc in accountList)
            {
                try
                {
                    ...
                    IJavaScriptExecutor js = (IJavaScriptExecutor)acc.browser.chromebrowser;
                    acc.browser.chromebrowser.Navigate().GoToUrl("https://listen.Napster.com/");
        ...
        public void StartPlaying()
        {
            foreach (var acc in accountList)
            {
                try
                {
                    ...
                    acc.browser.chromebrowser.Navigate().GoToUrl(""); //redacted
                }
                catch (Exception e)
                {
                    Console.WriteLine(e.ToString());
